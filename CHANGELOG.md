...
**不需要自建 NeteaseCloudMusicApi** —— 网易云网页端本来就有扫码接口，实测可用：

1. ``GET api/login/qrcode/unikey?type=1`` 拿 unikey
2. 把 ``https://music.163.com/login?codekey=<unikey>`` 画成二维码发出去
3. 轮询 ``api/login/qrcode/client/login``：800 过期 / 801 待扫 / 802 待确认 / 803 成功

803 时凭据既可能在响应体的 ``cookie`` 字段、也可能在 ``Set-Cookie`` 头里，
两种都兜住。成功后自动写进 ``music.neteaseCookie``，并回显账号昵称与会员状态。
...
网易云         ``api/nuser/account/get`` + ``music-vip-membership/front/vip/info``
QQ音乐         ``musicu.fcg`` 的 ``music.UserInfo.userInfoServer/GetLoginUserInfo``
              + ``VipLogin.VipLoginInter/vip_login_base``
============  ==========================================================

⚠️ QQ音乐这两个接口**必须带完整的 comm 块**（uin/authst/tmeLoginType/guid…），
只塞裸 comm 会稳定返回 ``code=1000``（未登录）。这是踩出来的。
...
- 背景默认取 ``https://api.elaina.cat/random/``，可在「插件设置」里换成别的
  地址，**留空则用纯色底**（更快）
- 毛玻璃：整幅背景**缩到 1/4 → 高斯模糊 → 放大**，只做一次模糊；
  面板是半透明**深色**圆角矩形。用深色而非白色是有原因的：白色面板配白字
  对比度极差，实测说明文字几乎看不清
- **花体昵称自动折叠**：QQ 昵称常用 ``𝓝𝓪𝓲𝓛𝓾𝓸`` 这类数学变体字母，
  Noto CJK 没有这些字形，直接画全是豆腐块。现在先把变体字母折回 ASCII
  （``𝓝𝓪𝓲𝓛𝓾𝓸`` -> ``NaiLuo``），再逐字检查字形是否存在、没有就丢掉
- 所有 Pillow 计算都在 ``asyncio.to_thread`` 里，不阻塞事件循环
- 背景抓取约 2.5 秒、头像约 2 秒，所以三个命令都**先把这些 task 起起来并行**，
  再一起收 —— 否则用户要等十几秒
- PNG 用 ``compress_level=3`` 保存，换渲染速度
...
新增 ``tests/test_panels.py``（本机能跑规则/布局/花体折叠，容器里额外跑真实渲染）。
## v1.4.2（2026-09-17）
