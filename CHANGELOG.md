...
**官机图集提速：抖音签名 CDN 的图不再白等转存（4~8 秒 → 1 秒内）。**
...
```
07:16:56.310  图集解析完成（2 张）
07:17:00.493  [转存] 试了 3 次都没成   ← 第 1 张
07:17:00.607  [转存] 试了 3 次都没成   ← 第 2 张
07:17:04.607  图集 1/2 张已转存，其余保留原始直链   ← 4.3 秒
```
...
**只有腾讯自己的下载器能取到** —— 而官机 markdown 恰恰就是让腾讯去下载。
所以对这类图做转存是**纯浪费**：
...
新增 `core.image_bed.SKIP_TRANSFER_HOSTS` + `should_skip_transfer()` ——
命中名单的图**直接跳过转存**，把原始直链交给腾讯：

- `_host_one()` 在调 `transfer_url` **之前**先判跳过
- `_host_images()` 的日志区分「主动跳过」与「试过没成」

**实测**（容器内跑真实代码）：

```
① 抖音图 _host_one -> ''         用时 0.00s   （原来 ~2.4s+）
② 普通图 _host_one -> 直链        用时 2.10s   （照常转存）
③ _host_images(1 抖音 + 1 普通)   用时 1.22s   （原来 4~8 秒）
```

### ⚠️ 一个前提

走这条路的图**完全依赖腾讯能取到**。之所以敢这么做，是因为
**v1.6.17 起图集就一直有一半图走「保留原链给腾讯」**，线上没出现过图挂。

**如果发现图集里某张图不显示** → 说明那个 host 腾讯也取不到，
把它从 `SKIP_TRANSFER_HOSTS` 去掉即可（拿不准就别加）。

### 顺带回答一个常见疑问

**官机「直发图片」本来就不走图床**：`Comp.Image` 走的是适配器
`upload_group_and_c2c_image`（腾讯富媒体接口），本地字节/路径直接传。
只有 **markdown 内嵌图**必须有公网 URL（腾讯服务器要去下载转存）——
菜单图、多图 MD 才需要转存。

新增 `tests/test_host_skip.py`（26 项）：名单命中、防误伤
（`notdouyinpic.com` / `douyinpic.com.evil.com` 必须**不**命中）、
「跳过必须发生在转存之前」。

## v1.6.19（2026-09-26）

**`#R菜单` 不再匹配裸词「菜单」—— 免得和别的插件抢事件。**

### 现象

`@机器人 菜单` 也能把菜单调出来。根因是规则里 `^#?`（`#` 可选）配上了裸词：
...
**`#菜单` 也一并不再命中** —— 只认带 `R` 的写法。这样别的插件要是也有「菜单」
命令，两边不会互相抢事件（`^#?` 让前缀可选，裸词落进自然语言里也容易误伤）。

- `#R菜单` / `R菜单` / `#R帮助` / `#R功能` 全部照旧
- 官机菜单按钮发的就是 `#R菜单`，不受影响
- `tests/test_panels.py` 补了断言：裸词 `菜单` 与 `#菜单` 都必须**不**命中

## v1.6.18（2026-09-25）

**B 站番剧（ep / ss）能解析了 —— 用「普通视频接口」绕开地区限制。**

### 现象（用户反馈）

官机发番剧链接 → 只回一句「番剧（ep/ss）解析未移植，需要 SESSDATA + BBDown」。

### 为什么以前没做

原版把番剧丢给 **BBDown** 下载（BBDown 自带 APP 端鉴权），本移植版没有 BBDown；
而官方的 `pgc/player/web/playurl` 在**大陆以外**会被地区限制（`rights.area_limit`）
压成 60 秒试看 —— 实测同一集（罗小黑战记第 1 话）在**洛杉矶服务器**上
`dash.duration` 只有 **62 秒**，在**广州家宽**是 **305 秒**。

### 解法

番剧单集本身就带 `bvid` / `cid`，所以绕开那个接口：

```
ep/ss 链接
  -> pgc/view/web/season     拿元信息（季标题 / 封面 / 简介 / 单集 bvid+cid+duration）
  -> x/player/wbi/playurl    用 bvid+cid 走**普通视频接口**（不吃地区限制）
  -> DASH + ffmpeg 合并      复用既有链路
```

实测洛杉矶服务器上这一步拿到 **305 秒完整 DASH（HDR + HEVC，18 轨）**，
下载合并出 **1920x1080 HEVC + AAC 的有效 mp4**。

### 行为（对齐原版「番剧直接解析」）

三道闸门，任一命中就**只发信息不下载**（带上封面 / 标题 / 简介 / 作品页链接）：

| 闸门 | 说明 |
|---|---|
| 未登录 | 番剧几乎都要登录，提示先 `#R登录 bilibili` |
| 超时长 | 用 `biliBangumiDuration`（默认 **1800** 秒，不是普通视频的 480） |
| 没开开关 | `biliBangumiDirect` 默认 **false** —— 开了才下载 |

画质走「番剧独立画质」`biliBangumiResolution`（下拉索引，默认 `5`），
与普通视频的 `biliResolution` 互不干扰。

### 其他

- `ep` 落在 `section`（PV / 花絮）里也能翻出来；短链（`b23.tv/xxx`）先展开再识别
- 短链现在**只展开一次**（原来番剧识别和 BV 提取各展开一次，白跑一个请求）
- 新增 `tests/test_bili_bangumi.py`（49 项）：链接识别（含「不误判 BV 号里的 ep」）、
  毫秒换算、三道闸门、画质分流、源码不变量（**不允许**出现官方番剧播放接口的 URL）

## v1.6.17（2026-09-24）

**修「图集退回逐条发送后没了标题」+ 转存改成逐张降级。**

### 现象（用户反馈）

官机发一条抖音**单图**图集 → 只出来图，**标题 / 作者 / 文案全没有**。

### 两条根因

**① 转存整体放弃得太早**（v1.6.15 引入）

`_host_images` 原本「任意一张转存失败就返回 `None`」→ 整条 MD 放弃 → 退回逐条发送。
而抖音签名 CDN 的图（`p3-pc-sign.douyinpic.com`）**本机 403、czoss 也 403**
（v1.6.7 实测过：换 p3/p6/p9 节点、加 Referer、带 1449 字符 Cookie 全是 403）
—— **只有腾讯自己的下载器能取**。所以「先转存」对抖音图**必然失败**。

改成**逐张降级**：转存成功的用国内直链；失败的那几张**保留原始 URL**
（留给腾讯去下载），整条 MD 照样发出。

**② 退回逐条发送时简介被跳过了**

`_render_direct` 在 `_will_send_gallery_md` 为真时会**跳过简介**（指望图集那条 MD
自带标题）。一旦 MD 拼失败退回逐条发送，那条标题就没了 —— 用户看到「只有图」。

新增 `_gallery_fallback_intro()`：退回时**补一条简介**。
只在 `_will_send_gallery_md` 为真时补，否则会和已发出的简介重复。

### 另外

`_host_one()`：直连转存失败时，会**本机下载**（自动补 Referer + 校验 Content-Type，
挡得住抖音的「200 + HTML 错误页」软失败）→ 传图床拿 URL → 再交给 czoss 转存。
对「本机能下、czoss 下不了」的图有效（抖音不在此列，它本机也 403）。

## v1.6.16（2026-09-24）

**图集 MD 定稿：三图一行 + 默认缩放 70%，标题与图合成一条。**

### 改动

* **三图一行**（`per_row = min(3, 张数)`）—— 之前是 2~3 张混着排，现在统一
  三图一行（不足三张就按实际张数）；
* **缩放改成百分比**：`mdMultiImageWidth`（绝对 170px）→ **`mdGalleryScale`**
  （默认 **70%**），基准是**单图宽度** `mdImageWidth`（300px）→ 默认每张 **210px**。
  用百分比是因为要调的就是「比单图小多少」这个相对量，以后改 `mdImageWidth`
  时图集跟着等比变，不用改两处；
* **标题 + 文案 + 图在一条 MD** —— 这条从 v1.6.7 起就是如此（`_send_album` 的官机
  分支发完就 `return`，且简介会跳过），本次把它写进测试锁住。
...
文案…
```
![图1 #210px #230px](…) ![图2 …] ![图3 …]
![图4 …] ![图5 …] ![图6 …]
```

一张 6 图的图集 = **一条消息、两行**，不再刷屏。点开 / 保存拿到的仍是原图。

## v1.6.15（2026-09-24）

**官机多图改版：所有图片先转存国内 OSS —— 一条 markdown、一行 2~3 张。**

### 为什么

官机**没有合并转发**（`Comp.Nodes` 适配器不认），N 张图只会：逐条发（**刷屏**），
或被 `max_images` 截断（用户实测过「三张图只出来一张」）。拼成一条 markdown
内嵌多图才对。

### ① 所有图片先转存到国内 OSS

官机 markdown 的图是**腾讯服务器下载转存**的（官方文档原话），而解析出来的
原始直链常常取不到 —— 抖音 `p3-sign.douyinpic.com` 要 `Referer`（本机实测
恒定 403）、各家 CDN 也未必对腾讯的下载器友好。

现在统一过一遍 `transfer_url`（czoss，落在广州电信），交给腾讯的是
**国内 + 内容固定 + https** 的地址。

新增 `_host_images()`：**并发**转存（9 张串行要 40 秒，并发 5 秒左右）；
**任意一张失败就整体放弃**，退回逐条发送 —— 不发一条半残（几张 `[alt]`）的 MD。
...
* 一行内用**空格**分隔 —— 这样才能横排；
* 行与行之间用**空行** —— 官方「换多行」明确：单换行不产生换行效果，
  紧贴的多行只渲染第一张。

### ③ 多图用更小的外显尺寸

新增配置 `mdMultiImageWidth`（默认 **170px**）；一行放三张时再等比缩一档
（×2/3）。**缩小只影响外显** —— 点开 / 保存拿到的仍是原图。

### 测试

新增 `tests/test_multi_image_md.py`（转存 / 排版 / schema 三节）。其中一条
断言来自这次踩的坑：**「单图宽度方法没被顶掉」** —— 用脚本做区间替换时把整个
`_md_image_width` 吃掉了，而**语法照样通过**，全靠回归测试才发现。
