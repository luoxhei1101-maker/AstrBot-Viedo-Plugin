...
实测 `api.czcn.xyz/api/czoss` **偶发失败**：有时干脆不返回，有时下载被截断
（只拿到几十 KB）。菜单一次没成，用户看到的就是「图没了」，所以现在
**默认重试 2 次**（间隔 0.8 秒），全失败才降级走图床、再不行降级成纯文字菜单。

连跑 10 次实测：**10/10 成功**，每次 1~2 秒，10 个 URL 全不重复。

重试全失败时会写一条 warning 日志（带源地址），线上排查降级原因不用猜。

## v1.6.12（2026-09-23）

**修复官机菜单「图片加载失败」**：菜单图改走**国内转存**，不再依赖海外图床。
...
freeimage.host 返回的 `iili.io` 挂在 Cloudflare 上 —— 广州本地 `curl`
`200 + image/jpeg`、六种 Referer 全通、无防盗链，但**放进官机 markdown
就报「图片加载失败」**。

> 教训：**「我这边 curl 得到」不等于「腾讯取得到」。**

已排除：图挂了 / 图片格式谎报（声明 jpeg 实为 PNG）/ hotlink 防盗链 /
按钮冲突。另外官机富媒体上传虽然能拿到 `file_info`，但拼
`multimedia.nt.qq.com.cn/download?appid=..&fileid=..` **一律 400**，那条路不通。

### 改法：转存到国内 OSS

```
取随机图（plugin.menuImageApi）
  → 交给 api.czcn.xyz/api/czoss 转存到国内 OSS     ← 新增
  → 拿固定 https 直链，读真实尺寸
...
| 项 | 结果 |
|---|---|
| 节点 | `113.96.129.5/7/8`（广州电信），`Server: ESA`（阿里云边缘加速） |
| 内容固定 | 同一链接连读两次 **408680B / 408680B** → 尺寸量得准 |
| 连转 3 次 | 3 个**不同**直链 → 菜单图每次都换，不被缓存 |

顺手少了一步：**转存接口能直接吃随机图 API 的地址** —— 原来那套
「下载字节 → 上传图床」不再需要了。

### 其它

* 新增配置 `czossKey`：**发行版不内置 key**。**想用官机菜单图，就先去
  <https://api.czcn.xyz> 的 API 详情页（`apidata?id=51`）创建 key**，
  填到插件配置的 `czossKey`（WebUI，或
  `data/config/astrbot_plugin_rconsole_config.json`）。没填也不会坏
  —— 转存失败会降级成纯文字菜单；
* 图床（`imageBedKey`）保留为**兜底**：转存不通时才会用到；
* 降级链：**转存 → 图床 → 纯文字 MD 菜单 → 本地渲染图**。

## v1.6.11（2026-09-23）

**官机菜单改版（一张图 + 一排按钮）**，并下架全部自检命令。

### 菜单：随机图 + 按钮

`#R菜单` 在官机上现在是一条 markdown：**上面一张随机图，下面一排按钮**。

图不是本地渲染的（本地 PNG 没有公网 URL，塞不进 markdown；而 `Comp.Image`
和 markdown 又互斥）。完整链路：

```
取随机图（plugin.menuImageApi，默认 elaina 竖屏档）
  → 本地读真实尺寸
  → 上传图床拿唯一链接（freeimage.host，key 可配置）
  → ![菜单 #300px #424px](url) + 按钮
```

**为什么必须过图床**：

* markdown 内嵌图**必须带尺寸**，而随机图 API 每次给的是**另一张**图 ——
  直接塞进 MD，尺寸对不上就会变形（实测 `/random/` 比例极差 **1.08**，
  `/random/mobile` 只有 0.10 → 默认用竖屏档）；
...
| 图床 | 结果 |
|---|---|
| **freeimage.host** | ✅ 免注册（有公开测试 key），返回 `iili.io` 直链，**国内可直读** |
| Telegraph | ❌ 上传返回 "Unknown error" |
| Catbox | ❌ Invalid uploader |
| 0x0.st | ❌ 官方已关闭上传 |
| sm.ms 匿名接口 | ❌ 空响应（需 token） |

降级链：**随机图 → 纯文字 MD 菜单 → 本地渲染的功能图**。

### 下架全部自检命令

`#R平台` / `#R平台测试` / `#RMD图` / `#RMD排版` / `#R按钮` —— 五条命令连同它们
的专用辅助方法一并移除，**命令规则 17 → 12 条**。调试期用的东西不该留在用户面前。

### 点歌降级提示改给「能保存」的直链

原来失败时给的是歌曲**详情页**（点开是网页、存不下来）。现在统一走
`_music_link()`：**优先音频直链**（就是语音用的那条 `play_url`），没有才退回详情页。

点播 MD 里也多了个按钮：**「保存音频」（直链）+「歌曲详情」（页面）** ——
一个能保存、一个不会过期。
...
| 键 | 默认 | 说明 |
|---|---|---|
| `plugin.menuImageApi` | elaina 竖屏档 | 菜单随机图 API；留空 = 不发图 |
| `plugin.imageBedKey` | 空 | 图床 key；空则用公开测试 key |

### 顺带修的两处

* **简介（解析文案）也走代码框**，标记写「标题」—— 作品文案几乎全是 `#话题`
  且分多行，当正文时每一行都会变成大标题；
* **图集场景跳过简介** —— 图集 MD 自己带标题，否则用户会看到两条标题
  （`_will_send_gallery_md`）。

### 测试

新增 `test_md_layout.py` / `test_image_bed.py` / `test_music_link.py`，
全量 **27 个文件**通过。

## v1.6.10（2026-09-23）

**官机图集改用「一条 markdown 内嵌多图」—— 顺带绕开了本机对抖音图的 403。**

### 症状

官机上发图集，用户侧什么都收不到，日志里只剩：

```
[R插件] 图片候选全部失败（2 个候选），末次错误: HTTP 403
  https://p3-pc-sign.douyinpic.com/...~tplv-dy-aweme-images:q75.webp?...
```

### 诊断：本机确实取不到这批图（不是代码问题）
...
|---|---|
| 原样（已带 Referer） | 403 |
| 去掉 `~tplv` 模板后缀（想拿原图） | 403 |
| 换 `p3` → `p9` → `p6` 节点 | 403 |
| 带用户的抖音 Cookie（1450 字符） | 403 |

URL 里的 `l=20260923132512…` 时间戳说明是**即时生成、即时失败**，不是签名过期。
结论：**抖音对这批图给出的签名本身就是无效的，本端无解。**

### 解法：把图交给腾讯服务器去取

markdown 内嵌图（`![](url)`）是**由腾讯服务器下载并转存**的 —— 我们完全不用下，
于是**绕开了本机 403**。这也是官机唯一能发「整组图」的形态（它没有合并转发）。

**尺寸是硬要求**。`![图1 #300px #286px](url)` 不带尺寸时，**电脑 QQ 照常显示、
手机 QQ 只渲染 `[alt]`**（v1.6.7 踩过）。好消息是**零额外请求** —— 抖音在
`images[i]` 顶层就给了：

```
=== images[i] 顶层键 ===
['download_url_list', 'height', 'is_aigc_media', 'is_new_text_mode',
 'resolution_log_param', 'uri', 'url_list', 'width']
  #1 (1612, 1538)  #2 (1612, 1538)  #3 (1612, 1538)  #4 (1612, 1538)
```

### 实现

- **`core/douyin_ssr.py`**：新增 `image_size()`（按「顶层 → `origin_url`」兜底，
  只接受正整数）；`album_items()` 的静态图项带上 `"size"`
- **`platforms/douyin.py`**：`extra["image_sizes"]`，与 `images` **一一对应**
- **`main.py`**：新增 `_album_md_text()` 负责拼 markdown；`_send_album()` 里
  「官机 + 纯静态图集」走这条路，而且**在下载之前就返回** —— 省掉整个下载过程
  （既快，也不再受 403 影响）

**只处理纯静态图集**：markdown 里塞不进视频，含动图的仍走原来的「逐条发」路径。

**任意一张缺尺寸就整体放弃**，退回逐条发送 —— 宁可慢一点，也不要发一条手机上
全是 `[alt]` 的消息。

### 实测（真实作品，已实际发送验证）

```
![图1 #300px #286px](https://p3-pc-sign.douyinpic.com/tos-cn-i-0813c000-ce/…)
![图2 #300px #286px](…)
```

1612×1538 等比缩到 300 宽 → 286 高（`_md_image` 里做的等比换算）。

### 测试

新增 `tests/test_album_md.py`（42 项断言）：`image_size` 的 10 种边界
（零 / 负数 / 字符串数字 / 非数字 / 非 dict）、`album_items` 带尺寸、
动图项**不应有** `size` 字段、尺寸传递链、以及「官机 + 纯静态」这个分支条件。

**全量 24 个测试文件通过。**

### 顺带

- `.gitignore` 补 `tests/_tmp_*/` —— 测试的工作目录之前会被 `git status` 看到。
