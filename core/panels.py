...
_CMD_DOC: dict[str, tuple[str, str]] = {
    ...
    "rMenu": ("#R菜单", "就是这张图"),
    "rConfig": ("#R配置 帮助", "管理员：平台开关 / Cookie / 点歌 / 发送形式"),
}
...
def build_menu_layout(version: str = "", bot_name: str = "") -> _Layout:
    ...
    # ---- 四、管理员配置 ----
    # #R配置 的子命令有十来条，菜单上只放最常用的四条：全列出来这张图会太长，
    # 「#R配置 帮助」里可以随时看完整的。
    lay.section("管理员配置（发 #R配置 帮助 看全部）")
    for name, desc in (
        ("#R配置 平台 抖音 开|关", "开关某个平台的自动解析"),
        ("#R配置 形式 聊天记录|直发", "解析内容是否打包成一条聊天记录"),
        ("#R配置 cookie 小红书", "私聊里设置 Cookie（我提示后再粘整串）"),
        ("#R配置 点歌 平台|数量|发送", "点歌默认平台、列表长度、发送方式"),
    ):
        lay.cmd(name, desc, (255, 209, 128))
    lay.gap()
    ...
    for text in (
        ...
        "3. Cookie：私聊发「#R配置 cookie 平台名」按提示粘贴；B站/网易云可直接扫码",
        "4. 少刷屏：管理员发「#R配置 形式 聊天记录」，解析结果合并成一条",
