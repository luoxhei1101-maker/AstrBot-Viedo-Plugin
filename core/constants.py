...
# 平台别名的正则片段。**长的必须写在前面**（正则交替是先匹配先赢，
# ``网易云|网易`` 才能正确命中「网易云」而不是先吃掉「网易」）。
MUSIC_PLATFORM_ALT = r"网易云|网抑云|网易|QQ音乐|qq音乐|QQ|qq"

# 点歌命令的匹配正则，**定义在这里供两处共用**：
# ``COMMAND_RULES`` 用它做粗筛，``main.py::cmd_music_search`` 用同一份提取
# 平台和关键词。两份正则一旦写歪，就会出现「命令命中了但解析不出关键词」
# 这种最难查的问题，所以只留一份。
#
# 支持的写法（平台可以在「点歌」之前或之后，也可以不写）：
#
# ==========================  ==========================================
# 输入                         行为
# ==========================  ==========================================
...
# ``网易云点歌 晴天``           强制网易云
# ``QQ点歌 晴天``              强制 QQ 音乐
# ``#点歌 网易云 晴天``         平台写在后面也认（兼容早期写法）
# ``/QQ点歌 晴天``             ``/`` 前缀同样接受
# ==========================  ==========================================
MUSIC_COMMAND_PATTERN = (
    r"^(?:#|/)?\s*"
    r"(?:(?P<pre>" + MUSIC_PLATFORM_ALT + r")\s*)?"
    r"点歌\s*"
    r"(?:(?P<post>" + MUSIC_PLATFORM_ALT + r")\s*)?"
    r"(?P<kw>.+)$"
)
...
COMMAND_RULES: tuple[dict, ...] = (
    ...
    # 平台能力自检。
    #
    # 为什么要做成命令：不同协议端（OneBot v11 / QQ 官方机器人）的消息能力
    # 差别很大，官方机器人**没有合并转发和音乐卡片**，但有原生 markdown。
    # 出问题时用户第一句总是「为什么不回/为什么没发出来」，有了这两条命令
    # 就能让他自己把「本机器人能发什么」贴出来，省一轮来回问。
    #
    ...
    },
    {
        "key": "resolveHelp",
        "name": "解析说明",
        "pattern": r"^#R(?:解析|resolve)$",
        "handler": "resolve_help",
        "admin": False,
    },
    # 点歌搜索。正则见上方的 MUSIC_COMMAND_PATTERN（main.py 共用同一份）。
    {
        "key": "musicSearch",
        "name": "点歌搜索",
        "pattern": MUSIC_COMMAND_PATTERN,
    ...
    # ⚠️ 只认带 `R` 的写法（`#R菜单` / `R菜单` / `#R帮助` / `#R功能`）。
    #
    # **不要**把裸词「菜单」加回来：`^#?` 让 `#` 可选，一旦把裸词列进去，
    # **@ 一下机器人 + 发「菜单」两个字就会触发** —— 别的插件也有菜单命令时
    # 会互相抢事件（2026-09-26 用户要求删掉）。同理也别加 `#菜单`。
    {
        "key": "rMenu",
        "name": "R菜单",
        "pattern": r"^#?(?:R菜单|r菜单|R帮助|r帮助|R功能)$",
        "handler": "r_menu",
        "admin": False,
    },
