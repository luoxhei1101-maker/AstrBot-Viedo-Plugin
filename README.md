...
> **关于「音乐卡片」的兼容性**：卡片走 OneBot 的 `music` 段，而 QQ 对不同类型的
> 卡片支持度不一样：
>
> | 平台 | 卡片类型 | QQ 展示情况 |
> |---|---|---|
> | QQ音乐 | `type=qq`（官方） | 一般正常，带播放按钮 |
> | 网易云 | `type=163` | **部分 QQ 客户端提示「发送者版本过低不展示」** |
>
> 这个提示来自 QQ 客户端/协议端，插件层绕不过去。如果网易云的卡片不展示，
> 建议把「发送方式」换成 **音频文件** 或 **文字 + 链接**。
