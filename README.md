...
>
> 原 Guoba 面板带来的 `#rns`（网易云状态）/ `#rks`（酷狗状态）/ `#rkq`（酷狗扫码）**已移除**——它们依赖的自建 API 服务在本移植版里都没实现。
> `#rnq`（网易云扫码）在 v1.3.0 一并移除过，**v1.5.0 用官方接口重新实现并恢复**（不需要任何自建服务）。

#### 三个图片命令

背景默认从随机图 API 取，**主体是毛玻璃面板**，文字带描边保证清晰。可在
「插件设置」分组里改 `panelBgApi`（留空则用纯色底，渲染更快）。

```
#R菜单       → 所有指令 + 使用教程
#cookie状态  → 哪些 Cookie 失效了、是谁的号、音乐会员到什么时候
#服务状态    → CPU/内存/磁盘/负载 + Bot 头像与账号
```

`#cookie状态` 只展示**平台 + 账号昵称 + 状态**，**不会回显任何 Cookie 内容**；
音乐平台额外给会员状态与等级，例如：
...
> QQ 昵称里常见的花体字母（`𝓝𝓪𝓲𝓛𝓾𝓸`）会自动折回普通字母显示，
> 不会出现豆腐块。
