# astrbot_plugin_rconsole

> 群里发一条分享链接，机器人自动把视频 / 图集 / 音乐扒出来发回来；
> 还支持点歌、扫码登录与三种状态查询图，解析结果也可以**打包成一条聊天记录**发。
>
> 本插件是 Yunzai-Bot 插件 **R插件**（[rconsole-plugin](https://gitee.com/kyrzy0416/rconsole-plugin)，作者 zhiyu1998）在
> [AstrBot](https://github.com/AstrBotDevs/AstrBot) 上的移植版。业务逻辑与设计思路来自原项目，
> **遵循原项目的开源许可协议 —— 木兰宽松许可证第 2 版（Mulan PSL v2）**，`LICENSE` 与原项目一致。
>
> 原项目 README 的声明同样适用：素材来源于网络，仅供交流学习使用，**严禁用于任何商业用途和非法行为**。

**当前版本：v1.6.17** ｜ 适配 AstrBot `>=4.16, <5`（在 v4.28.1 上验证）

<p align="center">
  <img src="https://q1.qlogo.cn/g?b=qq&nk=2593504303&s=640" width="104" height="104" alt="NaiLuo" />
  <br />
  <b>NaiLuo</b>　·　QQ 2593504303
  <br />
  <sub><i>「AI Agent！」</i></sub>
</p>

---

## 亮点速览

- **发链接即解析**，免 @ 机器人：抖音 / B站 / 快手 / 微博 / AcFun / 小红书 / 米游社 / 小黑盒 / 微视…
- **点歌**：`点歌 晴天` → 出一张列表图 → 回序号播放；网易云匿名即可用，QQ音乐配 Cookie 后 320kbps
- **扫码登录**：`#RNQ` 网易云、`#RBQ` B站，扫码后 Cookie 自动写进配置
- **三个状态图**：`#R菜单` 功能菜单、`#cookie状态` 一眼看清哪些 Cookie 失效、`#服务状态` 服务器负载
- **聊天里改配置**：`#R配置` 开关某个平台的解析、私信设置 Cookie、把解析内容切成「聊天记录」——不用开网页
- **OneBot v11 与 QQ 官方机器人各一份配置**：合并转发 / 音乐卡片 / markdown 按钮按协议端自动分流，
  官机上不会出现「配了却发不出去」的选项（详见[分协议端配置](#分协议端配置一个机器人一套)）
- **部署形态不受限**：AstrBot 与协议端是否同容器都能发视频（详见[推荐部署环境](#推荐部署环境)）

---

## 目录

- [必要环境](#必要环境)
- [安装](#安装)
- [推荐部署环境](#推荐部署环境)
- [使用](#使用)
  - [自动识别（免 @）](#自动识别免-)
  - [命令式指令](#命令式指令)
  - [三个图片命令](#三个图片命令)
  - [点歌](#点歌)
  - [扫码登录](#扫码登录)
- [配置说明](#配置说明)
- [Cookie 怎么填](#cookie-怎么填)
- [支持的平台](#支持的平台)
- [常见问题](#常见问题)
- [与原版的差异](#与原版的差异)
- [开发与测试](#开发与测试)
- [许可与致谢](#许可与致谢)

---

## 必要环境

### AstrBot 本体

- AstrBot `>=4.16, <5`（本插件在 **v4.28.1** 上验证通过）
- Python 3.10+（AstrBot 自带，无需单独安装）

### 外部工具（按需）

| 工具 | 是否必需 | 用途 |
|---|---|---|
| `ffmpeg` + `ffprobe` | ✅ 必需（B站高清） | B站 DASH 音视频分离后合并 / 转码 / m3u8 拼接 |
| `node` | 推荐（抖音动图/评论） | 生成抖音 `a-bogus` 签名（复用原版 `a-bogus.cjs`）。官方 Docker 镜像已内置 |
| `yt-dlp` / `BBDown` / `aria2c` / `tdl` | 可选 | YouTube 下载 / B站高画质 / 多线程下载 / Telegram |

> 插件启动时会自动探测并打印结果，缺哪个日志里会直接列出来。除 ffmpeg 外都是「用不到就不装」。
>
> **关于 `node`**：抖音的**动图解析**与**评论**依赖 `a-bogus` 签名。没有 node 时会回落到 SSR
> 免登录通道 —— 视频和静态图仍可解析，但**动图会被发成静图**（SSR 页面把动图的视频轨抹掉了）。
> 想完整支持动图请确保有 node + 抖音 Cookie。

### Python 库

| 库 | 用途 | 官方镜像 |
|---|---|---|
| `aiohttp` | HTTP（AstrBot 自带） | ✅ |
| `qrcode[pil]` | 扫码登录生成二维码 | ✅ |
| `Pillow` | 点歌列表图 / 状态图渲染 | ✅（12.3.0） |
| `psutil` | `#服务状态` 采集 CPU / 内存 / 磁盘 | ✅（7.1.3） |
| 字体 `NotoSansCJK-*.ttc` | 图片里的中文 | ✅ |

> 本地部署缺 `qrcode` / `Pillow` / `psutil` 时，插件**不会崩**：二维码发不出去会提示、
> 图片自动退回文字输出。这些库只影响对应功能，不影响链接解析主流程。

---

## 安装

进入 AstrBot 插件目录 `data/plugins`（Docker 部署即挂载的 `AstrBot/data/plugins`）：

```bash
# 国内（Gitee）
git clone https://gitee.com/MuLinYa259/astrbot-viedo.git astrbot_plugin_rconsole
# 海外（GitHub）
git clone https://github.com/luoxhei1101-maker/AstrBot-Viedo-Plugin.git astrbot_plugin_rconsole
```

两个仓库内容一致，二选一。不想装 git 就从仓库页下载 zip 解压，**目录名保持
`astrbot_plugin_rconsole`**。

### 安装后

1. 到 AstrBot WebUI 的「插件」页点**重载**（或重启 AstrBot）。
2. 打开插件配置页，按需填 Cookie、勾选平台。
3. 看启动日志：会打印「识别规则 X 条 / resolver X 个」+ 外部工具探测结果。

---

## 推荐部署环境

| 项 | 推荐 | 说明 |
|---|---|---|
| 系统 | **Linux（Debian / Ubuntu）+ Docker** | 容器化隔离、一键升级、免手工配依赖 |
| 内存 | **≥ 2GB**（下载合并高清建议 4GB+） | AstrBot 本体 ~260MB + 插件 |
| 磁盘 | ≥ 5GB 空闲 | 媒体临时文件 + 本地缓存 |
| GPU | 不需要 | 纯网络 + ffmpeg |

> 走云服务器时记得在**安全组**放行 AstrBot 管理端口（默认 6185）与 OneBot 端口 ——
> 安全组拦截是「端口连不通」最常见的原因。

**支持的消息平台**：aiocqhttp（QQ OneBot）、Telegram、Discord、飞书 / Lark、企业微信、
QQ 官方、Slack、钉钉。

### 部署形态：AstrBot 与协议端是否同容器

**两种形态都支持。**

插件发出的媒体**自带数据**交给协议端，不需要协议端去读 AstrBot 容器里的临时文件：

| 形态 | 支持 |
|---|---|
| AstrBot 与协议端在**同一容器** | ✅ |
| **两个独立容器、无共享挂载**（本项目实际的部署形态） | ✅ |

代价是消息体比原始文件大约三分之一，属可接受范围。

---

## 使用

### 自动识别（免 @）

群里**直接发链接**即可，不需要 @ 机器人。

| 平台示例 | 链接 |
|---|---|
| 抖音 | `https://v.douyin.com/xxxx/` |
| B站 | `https://b23.tv/xxxx`、`https://www.bilibili.com/video/BVxxxx`、纯 `BVxxxx` |
| 分享卡片（不限于 B站） | 从 App 分享到 QQ 的**卡片**：B站小程序、小红书图文、微博… |
| 快手 / 微博 / 小红书 / AcFun / 贴吧… | 对应的分享链接 |

> 之所以能免 @，是因为 AstrBot 的正则过滤器**不受 `wake_prefix` 约束**，与原版 Yunzai 的 rule 行为一致。

**分享卡片也能解析。** 在 QQ 里从 App 分享时，发出来的常常不是链接文本，
而是一张卡片（`CQ:json` 消息段）—— 这类消息的文本是**空的**，普通正则匹配不到，
所以插件单独走一条链路：从卡片原文里抠出链接，再过一遍平台识别规则表。

卡片里的链接字段各家不一样（B站是 `meta.detail_1.qqdocurl`，小红书是
`meta.news.jumpUrl`），插件**不按字段名取**，只认「卡片里有没有我们认识的链接」——
所以新增平台不用再为卡片写额外代码。

> 顺手一提：卡片原文会被记进日志（抹掉图片直链，只记录不发送）。遇到不认识的
> 卡片类型时，照着日志里的字段补规则表即可。

**发送形式可切换**：默认「直发」（简介一条、媒体逐条）；管理员可以切成
「聊天记录」——整条解析打包成**一条合并转发**，不刷屏，也不再单独发「识别成功」提示：

```
#R配置 形式 聊天记录      切成合并转发
#R配置 形式 直发         切回默认
```

### 命令式指令

| 指令 | 作用 | 权限 |
|---|---|---|
| `翻en <内容>` / `翻ja` / `翻kr` … | 翻译成指定语言（日/英/韩/法/德/俄/西） | 所有人 |
| `#总结一下 <链接>` | 抓网页正文，交给 AstrBot 已配的 LLM 总结 | 所有人 |
| **`#R菜单`** / `#R帮助` / `#R功能` | **图片版功能菜单**（所有指令 + 使用教程） | 所有人 |
| **`#cookie状态`** / `#ck状态` | **图片**：所有 Cookie 是否失效 + 昵称 + 音乐会员等级 | 所有人 |
| **`#服务状态`** / `#bot状态` | **图片**：服务器负载 + Bot 账号 / 头像 | 所有人 |
| `点歌 <歌名>` | 点歌搜索（网易云 / QQ音乐） | 所有人 |
| `网易云点歌 <歌名>` / `QQ点歌 <歌名>` | 指定平台点歌 | 所有人 |
| **`#R配置`** / `#Rc` | **在聊天里改配置**：平台开关 / Cookie / 点歌 / 发送形式（见下节） | 🔒 管理员 |
| **`#RNQ`** / `#rnq` | **网易云扫码登录**（官方接口，扫码即写入 Cookie） | 🔒 管理员 |
| `#RBQ` / `#rbq` | B站扫码登录 | 🔒 管理员 |
| `#RBS` / `#rbs` | 查当前 B站 Cookie 是否有效、是哪个号 | 🔒 管理员 |

> 🔒 标记的指令**只有 AstrBot 管理员能用**（名单在全局配置 `admins_id`）。普通成员触发时
> 默认**静默忽略**（不回消息，避免暴露指令存在），只记一条告警日志。
>
> 原 Guoba 面板带来的 `#rns` / `#rks` / `#rkq` **已移除** —— 它们依赖的自建 API 服务
> 在本移植版里没有实现。`#rnq` 在 v1.3.0 一并移除过，**v1.5.0 用官方接口重新实现并恢复**。

### 官机菜单（一张图 + 一排按钮）

QQ 官方机器人发 `#R菜单`，收到的是一条 **markdown**：上面一张随机图，
下面一排可点按钮（R菜单 / 视频解析 / 点歌 / R配置 / 状态 / 免艾特）。

图**不是**本地画的 —— 本地 PNG 没有公网 URL，塞不进 markdown；而按钮只能挂
markdown，`Comp.Image`（media 段）又和 markdown 互斥。所以走三步：

```
取随机图（plugin.menuImageApi，默认竖屏档）
   ↓  交给 api.czcn.xyz 转存到**国内 OSS**（plugin.czossKey 可选）
拿固定 https 直链，读出真实尺寸
   ↓
![菜单 #300px #424px](https://czoss.czcn.xyz/upload/xxx.jpg)  +  按钮
```

**为什么必须转存一趟**：

* 官机 markdown 的图是**腾讯服务器下载转存**的（官方文档原话）—— 所以图必须放在
  **国内能取到**的地方。实测海外图床（Cloudflare 上的 `iili.io`）本地 curl 全通，
  放进 markdown 却报「图片加载失败」；
* markdown 内嵌图**必须带尺寸**，而随机图 API 每次给的是**另一张**图 —— 直接
  塞进 MD，尺寸对不上就会变形（实测 `/random/` 的比例极差有 **1.08**，
  `/random/mobile` 只有 0.10，所以默认用竖屏档）。转存后 URL 内容固定，尺寸就量得准；
* 同一个 URL 会被客户端**缓存**，菜单图就永远是同一张了。转存每次生成新地址。

**官机发多图：一条 markdown、一行 2~3 张**

官机没有合并转发，逐条发图很刷屏。所以多图（图集 / 多图作品）会：

1. **尽量把图片转存到国内 OSS** —— 官机的图是腾讯服务器去下载的，转存成国内直链更稳。
   **抖音那类签名 CDN 本机和 czoss 都取不到**（403），这几张会**逐张降级**保留
   原始直链，交给腾讯去下载 —— 不会因为一张失败就整条 MD 不发；
2. 拼成**一条 markdown**：**标题 + 文案 + 图，三图一行**（一行内空格分隔、行间空行）；
3. 用 `mdGalleryScale`（默认 **70%**）控制每张的外显宽度 —— 基准是单图宽度
   `mdImageWidth`（300px），所以默认每张 **210px**。

> 缩小只是外显，**点开 / 保存拿到的仍是原图**。
> 万一图集 MD 还是没拼成，会退回逐条发送并**补一条简介**（标题不会凭空消失）。

**关于发视频（平台差异，容易踩）**

| 协议端 | 视频组件 | 原因 |
|---|---|---|
| **QQ 官方机器人** | `fromFileSystem`（**真实本地路径**） | 适配器 `Path(file).is_file()` 后读文件传腾讯；给 base64 会 `OSError: File name too long` |
| **NapCat 等** | `fromBase64` | 协议端在**另一个容器**，读不到这边的路径 |

两边**正好相反**，插件按平台自动选，用户不用管。

**挂了会降级**：转存 → 图床 → 纯文字菜单 → 本地渲染的功能图，不会什么都不发。
（转存接口偶发失败 —— 不返回、或下载被截断 —— 插件会自动**重试 2 次**。）

> 想换菜单图风格：把 `menuImageApi` 改成别的随机图 API（留空则不发图）。

**⚠️ 要用官机菜单图，先配一个 key**

发行版**不内置任何 key**，官机的菜单图需要你自己去创建一个：

1. 打开 <https://api.czcn.xyz> 的 API 详情页
   （[`apidata?id=51`](https://api.czcn.xyz/apidata?id=51)），创建 / 查看自己的 key；
2. 把它填进插件配置的 **`czossKey`**：
   * **WebUI**：插件配置 → R插件 → 「CZ API Key（官机菜单图必填）」；
   * **配置文件**：`data/config/astrbot_plugin_rconsole_config.json` 里的
     `plugin.czossKey`，改完重载插件。

没填 key 时菜单**不会坏** —— 插件仍会试着不带 key 转存，失败就自动降级成
纯文字菜单（按钮照旧）。**只是那张随机图很可能显示不出来。**

兜底图床（正常用不到）留了个 `imageBedKey`：到 freeimage.host 注册免费账号即可。

### 三个图片命令

背景默认从随机图 API 随机取，主体是**毛玻璃面板**，深色半透明叠在模糊背景上保证文字清晰。

```
#R菜单       → 所有指令 + 使用教程（内容自动从规则表生成，不会和代码漂移）
#cookie状态  → 哪些 Cookie 失效了、是谁的号、音乐会员到什么时候
#服务状态    → CPU/内存/磁盘/负载/运行时长 + Bot 头像与账号
```

`#cookie状态` 只展示**平台 + 账号昵称 + 状态**，**不回显任何 Cookie 内容**；
音乐平台额外给会员状态与等级，未配置的平台合并成一行：

```
已配置
  网易云音乐  幕Ming    黑胶VIP Lv.7　到期 2027-10-31
  QQ音乐      NaiLuo   豪华绿钻　Lv5
  抖音                 已配置（该平台暂无可用的校验接口）
未配置
  哔哩哔哩 · 快手 · 微博 · 小红书 · 米游社 · 视频号 · 小黑盒
```

> QQ 昵称里的花体字母（`𝓝𝓪𝓲𝓛𝓾𝓸`）会自动折回普通字母显示，不会出现豆腐块。
> 三个命令都把外部请求**并行发出**，通常 1–3 秒出图；把「图片命令的随机背景 API」
> 留空则改用纯色底，会更快。

### 管理员配置（`#R配置`）

常用开关不必再开网页翻配置分组，在聊天里就能改。**只有 AstrBot 管理员可用**
（名单在全局配置 `admins_id`），统一入口 `#R配置`（也认 `#R设置` / `#Rc`，
**必须带 `#` 或 `/`** —— 裸的「R配置」会误伤正常聊天）。

```
#R配置                      当前配置总览（发送形式 / 平台数 / 点歌 / Cookie）
#R配置 帮助                  完整用法

#R配置 平台                  列出所有平台的开关状态
#R配置 平台 抖音 关           关掉抖音的自动解析（改成「开」即打开）
#R配置 形式 聊天记录          解析内容打包成一条聊天记录
#R配置 形式 直发             改回「简介 + 媒体」多条消息

#R配置 cookie               各平台 Cookie 配置情况
#R配置 cookie 小红书         私信里设置（两步式，见下）
#R配置 cookie 小红书 <整串>   直接设置
#R配置 cookie 小红书 清除     清空

#R配置 点歌 平台 网易云        默认点歌平台（也认 QQ音乐）
#R配置 点歌 数量 10           列表显示几首（1–50）
#R配置 点歌 发送 卡片          link / card / voice / file
#R配置 点歌 方式 列表          list=列表图+序号点播；直接=搜到就送
#R配置 点歌 开关 开            点歌总开关
```

平台名**中文名和内部 key 都认**（`抖音` / `douyin`、`小红书` / `xhs`），
改完立刻回一句「现在是什么」，不用再去配置页确认。

#### 发送形式：直发 vs 聊天记录

| 形式 | 表现 | 适合 |
|---|---|---|
| **直发**（默认） | 简介单独一条，图片 / 视频一条条发 | 想让内容直接可见 |
| **聊天记录** | 简介 + 全部媒体打包成**一条合并转发**，也不再发「识别成功」提示 | 内容多、不想刷屏 |

一条链接只对应一条消息，点开是完整聊天记录，里面每个媒体各占一条「消息」，
可以单独转发或保存。整条解析里**没有任何媒体**时不会硬凑聊天记录，
会退回普通文字情报（一条只有文字的转发点开跟普通消息一样，还多一次点击）。

#### Cookie：私信两步式 + 必备字段体检

Cookie 是凭据，**只能在私聊里设置**（群里执行只回一句提示，不写入）：

```
你（私聊）：#R配置 cookie 小红书
机器人：⌛ 请在 3 分钟内，把 小红书 的 Cookie 整串发给我……
你（私聊）：web_session=xxx; a1=yyy; …      ← 直接粘贴
机器人：✅ 小红书 的 Cookie 已保存：913 字符 / 12 个字段
```

- **允许不完整**：不必把 F12 里的字段一个不漏抄下来，平台也常加新字段
- **但必须有必备字段**：一个都没有说明粘错了平台，会拒收并点名缺什么

| 平台 | 必备字段（至少一个） | 平台 | 必备字段（至少一个） |
|---|---|---|---|
| 哔哩哔哩 | `SESSDATA` | 网易云音乐 | `MUSIC_U` |
| 抖音 | `sessionid` / `ttwid` | QQ音乐 | `qqmusic_key` / `qm_keyst` |
| 小红书 | `web_session` / `a1` | 米游社 | `cookie_token` / `ltoken` / `account_id` |
| 快手 | `did` / `didv` | 小黑盒 | `x_xhh_tokenid` |
| 微博 | `_T_WM` / `MLOGIN` | 视频号 | `hy_user` / `hy_token` |

确实要强行写入（比如平台改了字段名）就加「强制」：
`#R配置 cookie 小红书 强制 <整串>`。

> 保存时会**顺手清空同平台的「逐项填写」** —— 它的优先级更高，留着旧值会把刚
> 设置的整串顶掉。设置完发 `#cookie状态` 可校验有没有真的生效。

##### 私信发了命令却没反应？先查 AstrBot 的会话白名单

这是**最容易踩的坑**，而且症状特别容易被误判成「插件坏了」：

```
你（私聊）：#R配置 cookie 小红书
（机器人：什么都不回）
```

**根因不在插件。** AstrBot 有一条 `whitelist_check` 流水线阶段，它在事件
**派发给插件之前**执行；私信会话不在白名单里就直接 `stop_event()` ——
插件**根本没收到这条消息**，自然不可能有回执。

日志里的特征是这一行（`docker logs astrbot | grep whitelist_check`）：

```
[Core] [INFO] [whitelist_check.stage:65]: Session ID
default:FriendMessage:1234567890 is not in the session allowlist,
so event propagation was stopped.
```

> **「群里能用、私信不能用」是正常现象** —— 群号往往早就在白名单里了，
> 而私信会话 ID 是另一套写法（`平台:FriendMessage:QQ号`），不会被一并放行。

**三种解法**（AstrBot WebUI → 配置 → 平台设置；改完**重启 AstrBot** 才生效）：

| 方案 | 怎么做 | 适合 |
|---|---|---|
| **管理员私信豁免**（推荐） | 打开 `wl_ignore_admin_on_friend` | 你的 QQ 已在 AstrBot 管理员名单里。一次设置，以后管理员的私信都放行 |
| 精确加白名单 | 把 `default:FriendMessage:<你的QQ号>` 加进 `id_whitelist` | 不是管理员，或只想放行某一个人 |
| 关掉白名单检查 | 关掉 `enable_id_white_list` | 不在意任何人私聊机器人（**不推荐**，白名单本身是有用的） |

会话 ID 直接复制日志里那串即可。白名单在 `cmd_config.json` 的
`platform_settings` 里，也可以手动改（注意该文件带 UTF-8 BOM）。

> 本插件在**群里**收到 Cookie 设置指令时，只会回一句「请私聊我发」并且
> **不写入**（免得凭据发到群里被别人拿走）。所以你在群里看到提示、
> 私信却毫无反应时，基本就是这个白名单在拦。

### 点歌

**首次使用需要打开开关**：插件配置 →「点歌」分组 → **开启点歌**（默认关闭）。

```
点歌 晴天            → 用配置里的「默认平台」
网易云点歌 晴天       → 强制走网易云
QQ点歌 晴天          → 强制走 QQ 音乐
```

平台写在「点歌」**前面或后面都认**，`#` / `/` 前缀可选（`#点歌 网易云 晴天` 也有效），
`QQ` 不分大小写。命令里写了平台就**只搜那个平台**，不会偷换；没写才用「默认平台」，
且该平台搜不到时会自动试另一个。

**搜索点歌与序号点播**（默认开启）：

```
点歌 晴天   → 返回一张列表图（默认 10 首）
1           → 60 秒内回复序号，播放第 1 首
```

- 列表图由插件现画（平台色条 + 封面 + 序号 + 歌名/歌手/专辑），**无二维码**
- 出图约 **0.6 秒**（v1.6.5 起；此前约 4.7 秒）。封面会走平台的缩略图接口
  并缓存，所以**重复点同一首歌更快**
- 序号只在**该会话 60 秒内搜索过**时才生效，不会干扰群里的普通数字消息
- **序号是一次性的**：点播成功后列表立刻失效，再发同一个序号不会有任何反应；
  序号越界只给提示、不失效，可以直接改发别的序号
- 想恢复「搜到就直接发」的老行为：把「点歌」分组的**点歌方式**改成**直接送**（`direct`）

**发送方式可在配置里切换**（「点歌」分组 → 发送方式）：

| 方式 | 效果 | 什么时候用 |
|---|---|---|
| **文字 + 链接**（默认） | 列出「歌名 - 歌手 + 播放页链接」 | 想自己挑，最稳 |
| **音乐卡片** | 发音乐分享卡，群里直接点播放 | 想要「点开就播」 |
| **语音条** | 下载后发语音条 | ⚠️ 只适合短音频，见下 |
| **音频文件** | 以群文件形式发音频 | 想听整首 |

> 后三种方式在发送前会**校验直链是否真的能播**（有些歌没有会员只能拿到
> 试听或根本取不到）。校验不过就自动改发「歌名 + 链接」，不会塞给你一个
> 打不开的文件。

#### 音乐卡片需要配一次签名代理

音乐卡片的 `token` 由协议端**外部的签名服务**签发（协议端不自己生成卡片）。
插件内置了一个签名代理用来对接它，**部署时把协议端的 `musicSignUrl` 指过来即可**：

```jsonc
// 协议端 config/onebot_<uin>.json 或全局 config/snowluma.json
{ "musicSignUrl": "http://astrbot:18888/" }
```

改完**重启协议端容器**。代理端口可在「点歌」分组里改（改了两边要同步）。

- 代理会自动对齐卡片品牌：网易云的歌显示「网易云音乐」，QQ 音乐的显示「QQ音乐」
- 只有「发送方式 = 音乐卡片」依赖它，其他方式不受影响

#### 两个音乐平台的差别

| | 网易云 | QQ音乐 |
|---|---|---|
| 搜索 | ✅ 匿名可用 | ✅ 匿名可用 |
| 配了 Cookie 后 | 解锁 VIP 歌高音质 | 解锁完整版（否则只能试听） |
| Cookie 门槛 | 只要 `MUSIC_U=xxx`，**可扫码获取** | 一整串，**必须有 `qqmusic_key`** |
| 接口稳定性 | 很稳 | **有随机限流，偶尔要重试** |

> QQ 音乐接口偶有限流，插件已内置重试与 10 分钟搜索缓存；
> 连续失败时换 `网易云点歌 歌名` 更省事。

#### 关于「语音条」的限制（重要）

语音条会被转成**未压缩音频**再传输，体积很大，一首正常长度的歌协议端收不下。
所以语音条只适合**短音频**，超了会提示并改用链接。**上限按协议端分档**：

| 协议端 | 上限 | 说明 |
|---|---|---|
| OneBot v11 | 90 秒 | 音频要经 HTTP body 传（base64 后约 117KB/秒） |
| QQ 官方机器人 | **300 秒** | 走官方富媒体接口，插件本地转 silk 后体积只有约 1.5KB/秒 |

**想发整首请选「音频文件」或「音乐卡片」** —— 这两个不受此限制。

> **官机的语音是插件自己转、自己传的**（v1.6.9 起）：本地用 `pysilk` 转成 silk
> 再走官方接口上传。这样做的原因是适配器那条路**上传超时不可控** —— botpy 的
> 超时是实例级属性、失败后还会重试 3 次，腾讯接口一抖用户就要干等 90 秒以上。
> 自己走之后超时 25 秒、失败立刻降级发链接（详见 CHANGELOG v1.6.9）。

### 扫码登录

| 指令 | 平台 | 说明 |
|---|---|---|
| `#RNQ` | 网易云 | 走网易云网页端官方扫码接口，**不需要自建 API** |
| `#RBQ` | B站 | 容器内 `qrcode` + `PIL` 本地生成二维码 |

流程：管理员发指令 → 机器人回二维码 → 用对应**手机客户端**扫码 → 手机上点「确认登录」
→ Cookie **自动写进配置**，并回一条账号信息（昵称 / UID / 会员状态）确认。

二维码约 3 分钟过期，超时可在配置里调（`plugin` → 「B站扫码等待超时」，默认 180 秒）。

---

## 配置说明

打开 AstrBot WebUI 的「插件配置」页。**原版的 Guoba 可视化面板已完整迁移**，
字段名沿用原面板，所以**原 Yunzai 的配置可以直接照搬**。

### 分组一览

| 分组 | 内容 |
|---|---|
| `plugin` | 移植版自己的开关：总开关、启用平台、图片背景 API、发送方式、下载并发… |
| **`profiles`** | **分协议端配置**：OneBot v11 / QQ 官方机器人 / 其它，各一份（见下节） |
| `global` | 全局黑名单、转发阈值、代理、视频编码、识别前缀 |
| `bili` | SESSDATA、画质、下载方式、显示项、评论 |
| `douyin` | Cookie、时长、压缩、评论、背景音乐 |
| `youtube` | 画质、时长、Cookie 路径 |
| `music` | **点歌**：开关、默认平台、列表长度、发送方式、网易云/QQ音乐 Cookie |
| `other` | 微博 / 小红书 / 视频号 / 快手 / 链接总结 |
| `xiaoheihe` | 小黑盒 Cookie |
| `ai` | 原版识图接口（本移植版复用 AstrBot 的 Provider，此项保留兼容） |
| `advanced` | 并发、清理计划 |

### 关键配置项速览

| 配置 | 默认 | 说明 |
|---|---|---|
| `plugin.enable` | 开 | 总开关 |
| `plugin.enabled_platforms` | 抖音/B站/通用/微博… | 只对勾选的平台生效。**没勾的平台链接会被静默跳过**（日志里会写「未在 plugin.enabled_platforms 里启用」）。例如快手默认不在列表里，要用记得勾 |
| `plugin.panelBgApi` | elaina 随机图接口 | 图片命令的随机背景；**留空则用纯色底，渲染更快** |
| `plugin.send_mode` | 直发链接 | `url`=直接发地址（快）；`download`=先下本地再发（稳） |
| `plugin.send_as_forward` | 关 | 解析内容是否**打包成一条合并转发**。等价于 `#R配置 形式 聊天记录` |
| `plugin.max_images` | 9 | 图集超过这个数就合并成「聊天记录」发送 |
| `plugin.download_concurrency` | 8 | 媒体并发下载数 |
| `music.enable` | 关 | 点歌总开关 |
| `music.sendMode` | 音乐卡片 | `link` / `card` / `voice` / `file` |
| `music.searchMode` | list | `list`=列表图+序号点播；`direct`=搜到直接送 |
| `music.enableSignProxy` | 开 | 音乐卡片签名代理 |
| `music.signProxyPort` | 18888 | 代理端口（与协议端 `musicSignUrl` 要一致） |

> ⚠️ **上表中带「发送形态」性质的项**（`send_mode`、`send_as_forward`、`max_images`、
> `album_forward_when_exceed`、`music.sendMode`、`music.searchMode`、`download_concurrency`、
> `mdImageWidth`、`qqButtons`、`enableSignProxy`）在 **v1.6.8 起改为按协议端分开配置** ——
> 见下面的「[分协议端配置](#分协议端配置一个机器人一套)」。那几项在这里的值只在
> `profiles.mode = 通用` 时生效；默认（分协议端）模式下读的是 `profiles.<协议端>` 那一份。
> 想让某个平台的旧值继续生效，把 `profiles.mode` 改成「通用」即可。

> **改过配置项位置的历史**：v1.3.0 起点歌从 `netease` 分组独立成 `music` 分组，并删掉
> 一批从未生效的遗留项（网易云自建 API / 云盘 / 发语音开关 / 全部酷狗配置）。
> 若从旧版升级，插件会**自动把旧位置的 Cookie 搬过来**；旧的 `netease` 分组会保留
> 一小段时间用于搬运（描述里标了「已废弃，可忽略」）。

### 分协议端配置（一个机器人一套）

同一份配置里，OneBot v11 和 QQ 官方机器人的**发送能力根本不是一回事**：

| | OneBot v11 | QQ 官方机器人 |
|---|---|---|
| 合并转发（聊天记录） | ✅ | ❌（适配器没有这个消息段） |
| 音乐卡片 | ✅ | ❌ |
| 原生 markdown | ❌ | ✅（还能挂消息按钮） |

所以「用聊天记录发送」「点歌发送方式=音乐卡片」这类选项，在官机上**怎么调都发不出去**。
v1.6.8 起把它们拆成**每个协议端一份**，放在 `profiles` 分组：

```
profiles
├── mode            ← 配置来源：分协议端（默认）/ 通用
├── onebot          ← OneBot v11 专用（12 项）
├── qqofficial      ← QQ 官方机器人专用（11 项）
└── fallback        ← 其它协议端（微信 / 钉钉 / Telegram…，9 项）
```

**AstrBot 的面板没有「选项卡」控件**（插件 schema 只支持
`bool/int/string/text/list/object/template_list`），所以这里用**可折叠分组**代替 ——
展开哪一组就改哪一组，效果等价。

**各组的默认值就是该平台最合理的值**，开箱即用：

| 配置 | OneBot | 官机 | 说明 |
|---|---|---|---|
| `send_as_forward` | 关 | — | 官机没有这一项（配了也不生效） |
| `album_forward_when_exceed` | 开 | 关 | 官机没有合并转发 |
| `music_send_mode` | `link` | **`voice`** | 官机没音乐卡片，语音是唯一能「听到歌」的形态 |
| `download_concurrency` | 8 | 4 | 官方接口对并发更敏感 |
| `md_image_width` | — | 300 | 官机专属：markdown 内嵌图宽度 |
| `qq_buttons` | — | 开 | 官机专属：菜单/列表下挂按钮 |
| `enable_sign_proxy` | 开 | — | OneBot 专属：音乐卡片签名代理 |

#### 三个要知道的点

1. **`mode` 选「通用」就回到升级前的行为** —— 所有值都从 `plugin.*` / `music.*`
   里的旧位置读。老配置不想动的话选它。

2. **升级不会改变你现有的行为。** 取值优先级是「面板里显式改过的 → 你改过的旧配置
   → 该协议端默认值」。所以以前设的 `plugin.send_as_forward=true` 照样生效，
   不会被新分组的默认值顶掉。

3. **能力高于偏好。** 官机上就算把 `music_send_mode` 设成 `card`，
   插件也会自动降级成 `voice` —— 不然就是发一条必然失败的消息。

#### 怎么查当前生效的是哪一份

```
#R配置 协议端      ← 按你说话的这个机器人回答，列出它这份的全部取值
```

`#R配置 形式` 和 `#R配置 点歌 发送` 这类命令**只改当前协议端那份**，
回执里也会写明「只影响 XXX 这份配置」。

---

## Cookie 怎么填

**不用自己拼格式。** 原插件要求手拼 `odin_tt=xxx;sessionid=xxx;...`，漏个分号就失败。
现在每个平台两种填法，二选一：

| 填法 | 适合 | 怎么做 |
|---|---|---|
| ① 整段 Cookie | 已复制一整段 | 直接粘进对应字段，原样透传 |
| ② 逐项填写 | 只想填关键几个 | 点「添加条目」→ 下拉选字段名 → 填值（行内表格） |

逐项填写用 AstrBot 的 `template_list` 控件，渲染成**行内表格**。插件按
`key=value; key=value` 组装，并做**必需项体检**（缺关键项会在日志里点名）。

### 支持逐项填写的平台

| 平台 | 至少填一个 | 备注 |
|---|---|---|
| B站 | `SESSDATA` | 整段字段只存单值，自动补 `SESSDATA=` 前缀；**也可扫码** |
| 抖音 | `sessionid` 或 `ttwid` | 原版列出的 12 个 key 全部可填 |
| 快手 | `did` 或 `didv` | 本移植版新增（原版快手不走 Cookie） |
| 微博 | `_T_WM` 或 `MLOGIN` | |
| 小黑盒 | `x_xhh_tokenid` | 整段字段只存单值 |
| 网易云 | `MUSIC_U` | **推荐直接用 `#RNQ` 扫码** |
| QQ音乐 | 整串（含 `qqmusic_key`） | 从 `y.qq.com` 复制整段 Cookie，插件自动映射字段别名 |

> 取法：浏览器登录后 F12 → Network → 任意请求 → Request Headers → Cookie 整段复制。
>
> ⚠️ QQ 音乐 Cookie 里的 `qqmusic_key`（也叫 `qm_keyst`）**有效期约 12 小时**。
> 过期后取不到音频，重新抓一次即可 —— 用 `#cookie状态` 能立刻看出来。

### 配了 Cookie 之后的变化

**B站（提升最大）**：未配 Cookie 走 360P 老接口；配了走 **WBI 签名 + DASH 高清**
（默认 1080P），音视频分离后用 ffmpeg 无损合并（`-c copy`）。WBI 签名在
`core/bili_wbi.py` 完整实现，不依赖第三方库。

**抖音**：双通道自适应 —— 有 Cookie + node 时走**主接口**（`aweme/detail` + `a-bogus`），
否则回落 **SSR 免登录**。

> **为什么动图必须配 Cookie**：实测 SSR 分享页会把动图**降级成静态图** ——
> `aweme_type` 从 `68` 改写成 `2`，`images[].video` 整个抹掉只留 `url_list`。
> 所以在 SSR 路径下无论怎么改判断都识别不出动图。新版 `share/slides` 链接更是
> 客户端 SPA（HTML 里没有 `_ROUTER_DATA`），**只能**走主接口。

> **为什么图集图片先下载到本地**：抖音图片直链（`p3-pc-sign.douyinpic.com/...`）是
> **带签名的 CDN 地址**，需要正确 `Referer`，而且同一作品里 `.webp` / `.jpeg`
> 两个候选**哪个 403 是随机的** —— 没有固定顺序可用。AstrBot 发送端自己的下载器
> 既不补 Referer 也没有候选回退，一张失败就会让**整条消息链**失败。所以一律先用
> 插件自己的「补 Referer + 候选回退」下载器落到本地，发送端只发本地数据。

**快手**：双路 —— 优先网页 SSR 直解，失败回落第三方接口。

---

## 支持的平台

`✅ 完整` = 核心流程已移植并验证｜`🟡 部分` = 主干可用，高级功能未搬｜
`⬜ 未移植` = 识别正常但会明确提示缺什么

| 平台 | 状态 | 说明 |
|---|---|---|
| 快手 / 西瓜 / 皮皮虾 / 皮皮搞笑 / QQ小世界 / 贴吧 / 即刻 | ✅ | 第三方接口轮换 |
| 微博 | ✅ | 正文 / 图集 / 视频，含 `mid2id` base62 转换 |
| AcFun | ✅ | ajaxpipe 抠 JSON → m3u8 |
| 哔哩哔哩 | ✅ 主干 | WBI 签名 + Cookie + DASH + ffmpeg 合并；**扫码登录**；评论（合并转发，**含评论图片**，v1.6.6）；**超时长会给出作品链接**（v1.6.4）；**番剧（ep / ss）已支持**（v1.6.18，需开「番剧直接解析」）；BBDown / 直播未移植 |
| AI 总结 / 翻译 | ✅ | 复用 AstrBot 自带的 LLM |
| 抖音 | ✅ 主干 | 主接口（a-bogus，配 Cookie）优先 + SSR 免登录兜底；动图/静态图逐项分流；**图集取原图不取压缩预览 + 多 CDN 候选回退**（v1.6.2）；评论（需 Cookie / node，**含评论图片与动图**，v1.6.6）；官机图集走 **MD 内嵌多图**（v1.6.10） |
| 网易云 / QQ音乐 | ✅ 含点歌 | 链接解析 + **`#点歌` 搜索 / 取直链 / 音乐卡片**；网易云**扫码登录**。歌单未移植 |
| 小黑盒 | 🟡 | 帖子解析（含 hkey 签名）；游戏页未移植 |
| 米游社 / 微视 | 🟡 | 主干已移植 |
| 汽水音乐 | 🟡 | 第三方直链 |
| 酷狗 | ⬜ | 老接口已失效、替代接口拿不到可分享直链，故移除（发链接会提示改用点歌） |
| 小红书 | ✅ | 需 Cookie（网页 SSR + `xsec_token`）；图文 / 视频；**QQ 分享卡片也能解析**（v1.6.1） |
| TikTok / X / Instagram | ⬜ | 需 TLS 指纹伪装 + Cookie |
| YouTube | ⬜ | 需 yt-dlp |
| 视频号 | ⬜ | 需腾讯元宝 Cookie |
| 波点 / 最右 / 小飞机 / AppleMusic / Spotify | ⬜ | 私有接口 / 外部服务 |

> 未移植的平台**不是静默失败**：发链接会收到明确的「尚未移植 + 卡在哪 + 需要什么」提示。

---

## 常见问题

**Q：发了链接没反应、也没日志？**
先查 AstrBot 全局配置 `cmd_config.json` 里的 `plugin_set` 是不是被改成了空数组 `[]`
（正常应是 `["*"]`）。这个配置会让所有插件的消息处理被跳过，症状就是「完全静默」。

**Q：某个平台的链接完全没反应？**
按顺序查三件事：① 插件的 `plugin.enabled_platforms` 有没有勾这个平台；
② 这个平台是不是在[支持列表](#支持的平台)里；③ 插件版本是不是旧的
—— 未命中规则 / 平台未启用 / 全局黑名单**三种跳过都会打日志**，直接看日志即可定位。

自 v1.6.0 起更省事：**启动日志会直接列出「已支持但没勾选」的平台**，
聊天里发 `#R配置 平台` 也能看到全部平台的开关状态，开一个：
`#R配置 平台 快手 开`。

**Q：在 QQ 里从 App 分享出来的「卡片」没反应？**
卡片（`CQ:json` 消息段）的文本是**空的**，走的是单独一条链路。自 v1.6.1 起
所有平台的卡片都会被识别，判断依据是「卡片里有没有我们认识的链接」。
排查方法：发一张卡片，看日志里有没有这两行 ——

```
[R插件] 收到卡片原始数据（用于补识别规则）：{...}
[R插件] 识别到 小红书 卡片: https://...
```

- 只有第一行、没有第二行 → 卡片里的链接域名不在识别规则表里
- **两行都没有** → 这张卡片没被当成卡片（例如平台本身没启用该平台解析）

**Q：小红书提示「链接里缺少 xsec_token」？**
这个参数是取内容的**硬前提**，且与链接绑定、有时效。

- 从 App 里走「分享 → 复制链接」拿到的链接带着它；
- 直接复制浏览器地址栏的链接**通常不带**，所以会失败；
- 如果是 **QQ 分享卡片**出现这个提示，说明卡片里的 `jumpUrl` 本身没带该参数
  （正常情况是带的）—— 这种只能请对方改用复制链接的方式。

**Q：第三方解析接口挂了？**
这类接口生命周期很短，默认的通用接口里只有部分还可用。全挂了就去
`plugin.parse_endpoints` 换新的，不用改代码。代码对 DNS 失败做了快速失败，
不会在死域名上白等重试。

**Q：重复发同一个链接会重新解析吗？**
不会。有**作品缓存**（2 小时），命中直接重发，跳过网络解析。

**Q：音乐卡片显示「发送者版本过低，无法展示内容」？**
说明协议端没能拿到有效的卡片签名。检查两件事：① 协议端 `musicSignUrl` 是否指向
本插件的代理（`http://astrbot:18888/`）；② 改完配置有没有**重启协议端容器**。
见[音乐卡片需要配一次签名代理](#音乐卡片需要配一次签名代理)。

**Q：`#cookie状态` 显示某个平台「已配置（暂无可用的校验接口）」？**
抖音 / 快手 / 小红书 / 视频号没有公开的账号校验接口，插件不会谎报「有效」，
只说明「已配置」。其余平台都是真实调用接口判断的。

**Q：点歌的语音条发不出来？**
正常现象，见[语音条的限制](#关于语音条的限制重要)。改用「音频文件」或「音乐卡片」。

**Q：抖音图集总有几张发不出来 / 发出来是糊的？**
自 v1.6.2 起已修。抖音图集每张图的 `url_list` 里有多个候选，分工不同：
`.webp` 是**压缩预览**（质量 75 模板，体积只有原图一半），`.jpeg` 才是**原图**；
而且 403 出现在哪个候选是**每张图固定的**（不是随机的，多试几次不会变好）。
旧版按 `url_list` 原顺序「试到第一个成功就用」，所以经常拿到模糊的 `.webp`，
候选少的图两个都 403 就整张丢掉。现在改成**原图优先排序 + 逐个候选回退**，
并在下载层拦掉抖音 CDN 那个「200 + text/html 的 238 字节错误页」。
实测同一图集总量从 425KB → 989KB（4 张从 webp 换成 jpeg）。

**Q：为什么不能像原版那样把图片 URL 直接交给协议端发？**
原版看起来「全都成功」，是因为它**根本不下载**——把 URL 交给协议端，抓失败也不
报错、不阻塞其它图，所以观感是「全出来了」，但发出去的其实也是 `.webp` 压缩图，
失败的图在 QQ 里就是个空白框。AstrBot 这边发送端的下载器**不带 Referer、没有
候选回退、也不校验内容**，直发时只要一张失败就抛错，**整条消息链一起失败**
（现象是「一条图都没发出来，只剩简介文字」）。所以本插件一律**先落盘再发本地文件**。

**Q：发了个 B 站链接，机器人一点反应都没有？**
大概率是**视频时长超过了配置上限**（`biliDuration`，默认 480 秒 = 8 分钟）。
自 v1.6.3 起这种情况会明确回一句原因，v1.6.4 起还会**把作品链接一起发出来**：

```
🔗 哔哩哔哩
标题：东尼爆改麦晓雯！男人也可以这么美丽吗？！【bilibilionly同人扶持计划】
作者：流萤Zz
⏱️ 视频时长 9分35秒 超过上限 8分0秒，未下载（可在插件配置里调整 biliDuration）
👉 观看地址：https://www.bilibili.com/video/BV1awbW6cESg
```

这样不用自己拿标题去搜。旧版（v1.6.2 及以前）是**静默的** —— 这类「解析出来了、
但按规则不发」的情况当时和「网络报错」混在一起，统一由 `reply_on_error` 控制，
而该选项默认关闭，所以只能翻日志才知道原因。

想放宽上限就在插件配置里把 `biliDuration` 调大（单位秒；填 `0` 表示不限制）。
注意关掉时长限制后，几小时的直播回放也可能会被拉下来，占带宽和临时目录。
网络类失败（超时 / 接口报错）仍然默认静默，照旧由 `reply_on_error` 控制。

**Q：评论里的图片能一起提取出来吗？**
能，v1.6.6 起。发出去的样子：

- **静态图**：文字在上、图片在下，放**同一条**转发记录里；
- **动图**：转成 mp4 后**单独发一条**（有文字的话文字也单独一条）——
  混在一起 QQ 那边渲染顺序会乱。评论里的动图是**表情包（）**，
  它既不在作品图集里也不在视频字段里，v1.6.7 起已能提取；
- **纯图评论**（作者经常只发图不配字）也能正常发出来，正文只留一行时间/地点/赞数。

图片和作品图集一样是**先下到本地再发**，而且取的是**原图**（抖音评论图的
`origin_url` 实测 1600×1600，`.jpeg` 候选比 `.png` 小 20%）。

需要满足：评论开关打开（`biliComments` / `douyinComments`），
抖音还需要 `node`（生成 `a-bogus` 签名）并建议配 Cookie。

---

## 与原版的差异

移植时把逻辑拆成了「平台解析（`platforms/`）」与「基础设施（`core/`）」两层，
解析器不依赖框架，可以脱离 AstrBot 单独测试 —— 这也是能在部署前验证逻辑的原因。

功能上的主要差异：

1. **LLM 复用 AstrBot 的模型**：不用再填 `aiBaseURL` / `aiApiKey` / `aiModel`。
2. **启动自检**：探测 ffmpeg / node / yt-dlp / BBDown / aria2c / tdl，日志直接列缺什么。
3. **作品缓存**：重复链接 2 小时缓存，命中直接重发，跳过网络解析。
4. **先发简介再发媒体**：标题 / 作者 / 类型先返回，大文件的合并 / 转码延后。
5. **不静默失败**：未移植的平台、被跳过的平台都会给明确提示或日志；
   被配置规则拦下的（如视频超时长）会说出原因**并给出作品链接**（v1.6.4），
   因为它恰恰是用户改个配置就能解决的、或至少能换个地方看。
6. **新增原版没有的能力**：点歌（搜索 + 列表图 + 序号点播 + 音乐卡片）、
   网易云 / B站扫码登录、三个状态图、`#R配置` 聊天内改配置、解析内容合并转发。
7. **发送形式可选**：原版固定「简介 + 媒体」多条直发；本移植版可切成一条合并转发。

针对「等待」做了几处优化：HTTP 连接池复用、签名进程常驻、画质探测并发、
图片命令的外部请求全部并行（十几秒 → 1–3 秒）。

---

## 开发与测试

测试全部**离线**、不依赖网络与 Cookie，直接跑就行：

```bash
python tests/test_album_send_path.py            # 图集发送路径
python tests/test_douyin_album.py               # 抖音图集动图 / 静图分流
python tests/test_douyin_album_candidates.py    # 抖音图集「原图优先」候选排序
python tests/test_downloader_media_guard.py     # 下载器的软失败防护（403 错误页等）
python tests/test_reject_notice.py              # 按规则拒绝（超时长）必须提示用户
python tests/test_xiaohongshu.py                # 小红书解析（webId 剔除、画质挑选）
python tests/test_forward_send.py               # 聊天记录转发发送形式
python tests/test_r_config.py                   # #R配置（平台开关 / 发送形式 / Cookie）
python tests/test_music_search.py               # 点歌搜索 + 命令正则
python tests/test_music_config.py               # 配置迁移
python tests/test_music_pick.py                 # 序号点播
python tests/test_music_card_image_perf.py      # 点歌列表图出图性能（小图 URL / 缓存 / 线程池）
python tests/test_comment_image.py              # 评论图片 / 动图提取（纯图评论不再被丢）
python tests/test_platform_caps.py              # 协议端能力表（合并转发 / 卡片 / MD / 按钮）
python tests/test_platform_profiles.py          # 分协议端配置（含 schema 一致性）
python tests/test_qq_buttons.py                 # 官机按钮 / markdown 内联指令
python tests/test_qq_voice.py                   # 官机语音：silk 编码 + 自控上传（真跑 pysilk）
python tests/test_album_md.py                   # 官机图集：MD 内嵌多图 + 尺寸兜底
python tests/test_md_layout.py                  # 官机 markdown 排版（代码框 / 空行 / 标题）
python tests/test_image_bed.py                  # 菜单配图（国内转存 + 图床兜底）
python tests/test_music_link.py                 # 点歌降级链接（优先音频直链，不是详情页）
python tests/test_music_sign_proxy.py           # 音乐卡片签名代理
python tests/test_music_url_guard.py            # 音频直链可用性校验
python tests/test_panels.py                     # 三个图片命令
python tests/test_a_bogus_worker.py             # a-bogus 常驻 worker
python tests/test_http_pool_bench.py            # 连接池基准
python tests/test_card_link.py                  # 分享卡片链接提取
```

**`test_r_config.py` 里有一条特别值得留意的断言**：它用 AST 扫出所有
`self._save_conf("a.b", ...)` 的路径，逐个去 `_conf_schema.json` 里查是否存在 ——
AstrBot 会在插件代码执行前**按 schema 裁剪配置**，schema 里没有的键写进去当时有效、
下次启动就被删掉（v1.3.0 丢过一整套 Cookie）。以后新增可写配置项，这条断言会
直接把漏改 schema 的情况拦下来。

`test_platform_profiles.py` 把同一条守得更死：**代码里的每个 `profiles.*` 字段
都必须能在 schema 里找到**，而且两边默认值要一致 —— 不一致的后果是
**面板显示一个值、插件实际用另一个值**，用户怎么调都调不对。
它还比对了 `SHARED_DEFAULTS` 与 schema 里旧键的默认值（不一致会让
「用户改没改过」的判断失准）。

几个测试做了**环境自适应**：本机自动桩出最小 AstrBot API，容器里则用真实 AstrBot。
所以同一份文件可以直接丢进容器跑，**验证的是已部署的那份代码**：

```bash
docker exec astrbot python /AstrBot/data/plugins/astrbot_plugin_rconsole/tests/test_panels.py
```

---

## 许可与致谢

### 许可

本项目是 [rconsole-plugin](https://gitee.com/kyrzy0416/rconsole-plugin) 的移植版本，
业务逻辑与设计思路来自原项目，遵循原项目的开源许可协议 ——
**木兰宽松许可证第 2 版（Mulan PSL v2）**。

原项目 README 中的声明同样适用：素材来源于网络，仅供交流学习使用，
**严禁用于任何商业用途和非法行为**。

### 参考与依赖的开源项目

| 项目 | 作者 | 用到了什么 |
|---|---|---|
| [rconsole-plugin](https://gitee.com/kyrzy0416/rconsole-plugin) | zhiyu1998 | **本插件的源项目**。全部平台解析逻辑、配置项设计、Cookie 字段定义都来自它 |
| [AstrBot](https://github.com/AstrBotDevs/AstrBot) | Soulter 及贡献者 | 插件框架本体。消息组件、事件模型、配置 schema、LLM Provider 均复用其能力 |
| [NapCatQQ](https://github.com/NapNeko/NapCatQQ) | NapNeko | QQ 协议端。音乐卡片的签名机制与默认签名服务地址参考其实现 |
| [SnowLuma](https://github.com/motricseven7/snowluma) | motricseven7 | 本项目实际使用的 QQ 协议端 |
| [xiaofei-plugin](https://github.com/xfdown/xiaofei-plugin) | xfdown | 点歌功能的多平台抽象与 QQ 音乐取流流程参考 |
| [QQMusicApi](https://github.com/luren-dc/QQMusicApi) | luren-dc | `#cookie状态` 里 QQ 音乐会员等级接口的模块名参考 |

### 外部服务

以下服务**不属于本项目**，插件只是调用它们。它们由第三方提供、随时可能变动或失效：

| 服务 | 用途 |
|---|---|
| 音卡签名服务（yibai / 思思） | 音乐卡片签名，NapCat 官方默认值 |
| `api.elaina.cat/random/` | `#R菜单` / `#cookie状态` / `#服务状态` 的随机背景图 |
| `q1.qlogo.cn` | `#服务状态` 里的 Bot 头像 |
| 各平台官方接口 | 链接解析、Cookie 校验、扫码登录 |

这些服务不可用时插件会**明确报错或降级**（退回纯色底、退回文字输出），不会静默失败。

### 依赖库

[Pillow](https://python-pillow.org/)（图片渲染）、[psutil](https://github.com/giampaolo/psutil)（系统信息）、
[aiohttp](https://github.com/aio-libs/aiohttp)（HTTP）、[qrcode](https://github.com/lincolnloop/python-qrcode)（扫码二维码），
以及 **Noto Sans CJK**（图片中的中文字体）。

感谢以上所有项目的作者与维护者。

---

## 免责声明

- 本插件仅供**学习与技术交流**，请勿用于任何商业用途或非法用途。
- 使用本插件解析、下载的内容版权归原作者所有；请遵守各平台的服务条款。
- 扫码登录获得的 Cookie 保存在**你自己的 AstrBot 配置**中，本项目不上传、不收集。
- 因使用本插件产生的一切后果由使用者自行承担。
