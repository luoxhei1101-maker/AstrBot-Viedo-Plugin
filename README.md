...
| `#点歌 <歌名>` | **点歌搜索**，列出候选（歌名 + 歌手 + 播放页链接） | 所有人 |
...
**首次使用需要打开开关**：插件配置 →「网易云音乐」分组 → **开启点歌功能**（默认关闭）。

```
#点歌 晴天            → 用配置的默认平台搜，列出 10 首候选
#点歌 网易云 晴天      → 只搜网易云
#点歌 QQ音乐 晴天      → 只搜 QQ 音乐
```

输出长这样（只发文字 + 网页链接，**不发音频文件**，不占带宽也不用转码）：
...
**QQ 音乐 Cookie 怎么填**：浏览器登录 <https://y.qq.com> → F12 → Network → 任意请求 →
Request Headers → 把 `Cookie:` 整串复制，粘到「其他平台」分组的 **QQ音乐Cookie** 里。

不用自己改字段名——插件会自动映射（`uid`→`uin`、`qqopenid`→`psrf_qqopenid` 等）。

> ⚠️ QQ 音乐 Cookie 里的 `qqmusic_key`（也叫 `qm_keyst`）**有效期约 12 小时**。
> 过期后点歌的链接点开只能试听，重新抓一次 Cookie 即可。这是 QQ 音乐的机制，不是插件问题。
