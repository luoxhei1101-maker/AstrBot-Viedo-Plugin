...
| 🔒 **B站扫码登录** | `#RBQ` | **仅管理员**。发二维码，扫码后 Cookie 自动写进配置 |
| 🔒 **B站账号状态** | `#RBS` | **仅管理员**。查当前 Cookie 是否有效、是哪个号 |

### 🔒 受限指令只有 AstrBot 管理员能用

带 🔒 的指令判定走的是 **AstrBot 官方的管理员名单**（全局配置里的 `admins_id`）：
先用 `event.is_admin()`，再兜一层直接读 `admins_id` 比对 sender_id——
语义和 `@filter.permission_type(PermissionType.ADMIN)` 完全一致。

普通成员触发时：**默认静默忽略**（不回消息，避免向群里暴露"这里有个管理员指令"），
同时在日志里记一条告警。想让它回一句「仅限管理员使用」，
把配置里 `plugin` → **非管理员触发受限指令时静默忽略** 关掉即可。

### B站扫码登录（`#RBQ`）

1. 管理员在群里发 `#RBQ`
2. 机器人回一张二维码（用容器内的 `qrcode` + `PIL` 本地生成，不依赖任何外部服务）
3. 用 **B站手机客户端** 扫码 → 手机上点「确认登录」
4. `SESSDATA` / `bili_jct` / `DedeUserID` / `buvid3` **自动写进插件配置**，
   并立刻回一条账号信息（昵称 / UID / 等级 / 会员状态）确认生效

轮询跑在后台任务里，不会把消息处理卡住；插件卸载时会主动取消，不留野任务。
二维码约 3 分钟过期，超时时间可在配置里调（默认 180 秒）。

> 写入时会清空「整段 Cookie」那栏 —— 因为它优先级更高，留着旧值会把刚扫出来的
> 新凭据盖掉。单一数据源，避免"扫了码却没生效"这种鬼打墙。
...
| ② **逐项填写** | 只想填关键的几个 | 点「添加条目」→ **下拉选字段名** → **输入框填值**，一行一个字段 |

逐项填写用的是 AstrBot 的 `template_list` 控件，前端渲染成**行内表格**——
顶部「添加条目」，下面一排排条目，每行一个「字段名下拉 + 值输入框」。
（之前用的 `dict` 编辑器是弹窗式的，加一个键还得先选 value 类型，难用，已换成这个。）

插件按 `key=value; key=value; ...` 组装，**按你在表格里排好的顺序**。
还会**做必需项体检**——缺关键项时在日志里点名，不用你去猜为什么解析失败。
...
| 哔哩哔哩 | ✅ 主干完整 | **WBI 签名 + Cookie + DASH + ffmpeg 合并**全通；`#RBQ` 扫码登录拿 Cookie；BBDown / 番剧 / 直播 / 评论截图未移植 |
...
| 各平台扫码登录 | 🟡 B站已支持 | B站 `#RBQ` / `#RBS` 已可用；网易云 / 酷狗 的扫码仍是占位 |
